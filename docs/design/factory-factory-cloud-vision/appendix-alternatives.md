...
We evaluated two architectural approaches before choosing Approach #2
(Library-based). This appendix documents both approaches and the reasoning
behind the decision.
...
- Higher resource cost: Full FF server (Node.js + SQLite) per user (~200MB per
  VM)
