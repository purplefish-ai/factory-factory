...
This directory contains stage-by-stage design documents for extracting
`@factory-factory/core` from the monolithic Factory Factory codebase, as
outlined in the [Cloud Vision](../factory-factory-cloud-vision.md) document.

## Goal

Convert the single-package repo into a pnpm monorepo with a publishable
`@factory-factory/core` library containing workspace, session/claude, and
ratchet domain logic behind storage and infrastructure abstractions. The desktop
app continues to work identically, importing core as a workspace dependency.
...
- **GitHub domain stays in desktop** -- wraps `gh` CLI; cloud implements its own
  bridge
- **Terminal and run-script stay in desktop** -- PTY/native modules, local
  process management
- **`src/shared/` is not a separate package** -- types needed by core move
  there; frontend re-exports
- **Core has zero Prisma dependency** -- uses storage interfaces implemented by
  consumers
- **Core has zero singleton dependency** -- logger, config, etc. injected via
  interfaces
