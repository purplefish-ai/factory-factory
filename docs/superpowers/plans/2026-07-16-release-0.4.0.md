...
**Goal:** Prepare and publish a draft pull request that bumps Factory Factory
from 0.3.21 to 0.4.0 and documents the changes merged since v0.3.21.

**Architecture:** This is a release-metadata-only change. Update the root
package version and prepend a curated Keep a Changelog entry; do not change
runtime code, dependencies, generated files, or the private core package
version.

**Tech Stack:** JSON package metadata, Markdown changelog, pnpm validation, Git,
GitHub CLI
...
- Consumes: Existing root package version `0.3.21`, Keep a Changelog formatting,
  and commits in `v0.3.21..HEAD`.
- Produces: Root package version `0.4.0` and a dated `0.4.0` changelog section
...
Do not change `packages/core/package.json`, which remains private at version
`0.1.0`.

- [x] **Step 3: Add the curated changelog entry**

Insert the following section immediately before `## [0.3.21] - 2026-06-22`,
preserving the existing document header:
...
Expected: All commands exit with status 0; the diff shows `0.4.0` in the root
package and the new `2026-07-16` changelog section.
