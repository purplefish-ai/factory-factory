...
**Goal:** Update WebSocket-backed project views immediately when a PR URL is
persisted even if GitHub snapshot retrieval fails.

**Architecture:** Introduce a URL-only GitHub domain event and bridge it through
the event collector into an immediate partial `WorkspaceSnapshotStore` upsert.
Preserve the existing full snapshot event contract and existing `fetch_failed`
mutation behavior.

**Tech Stack:** TypeScript, Node.js `EventEmitter`, Vitest, tRPC,
...
- Preserve `{ success: false, reason: 'fetch_failed' }` for both attachment
  paths.
...
Import `PR_URL_ATTACHED` and `PRUrlAttachedEvent`. In the manual `fetch_failed`
test, register a listener and assert it receives exactly:
...
Add the same assertion to the guarded discovery `fetch_failed` test. Extend the
missing-workspace and stale-claim cases with listeners that remain uninvoked.
...
Expected: PASS with one URL event after each persisted `fetch_failed` case and
...
- Test:
  `src/backend/orchestration/event-collector-lifecycle.integration.test.ts`
...
Extend the GitHub module mock with `PR_URL_ATTACHED: 'pr_url_attached'`. Assert
collector startup registers the new listener, stop removes it, and a URL event
invokes:
...
Import the event constant and type from `@/backend/services/github`. Register a
handler that calls:
