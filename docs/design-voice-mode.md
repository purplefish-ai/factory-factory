...
4. **`voiceMode` needs to reach every place a `QueuedMessage` is read, not just the happy path.** `RecentMessageRejection.userMessage` (`session-store.types.ts:11`, a `Pick<QueuedMessage, 'text' | 'timestamp' | 'attachments'>` used by `failMessage` to restore a failed send back into the composer for retry) doesn't currently carry `settings` or `voiceMode` at all. If a voice message fails and gets restored for retry, decide whether `voiceMode` should survive that round-trip (probably yes, so a retried voice message still gets the brevity instruction) — if so, add `'voiceMode'` to that `Pick` too.
