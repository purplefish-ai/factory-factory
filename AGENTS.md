...
- **ACP Runtime:** All agent sessions use the Agent Client Protocol (ACP) via `@agentclientprotocol/sdk`. Each session spawns an adapter subprocess (e.g., `claude-adapter`, `codex-adapter`) with stdio-based JSON-RPC communication. Permission requests present multi-option selection (allow_once, allow_always, deny_once, deny_always). Model/mode/reasoning controls driven by agent-provided configOptions.
