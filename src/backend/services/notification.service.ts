...
async function sendLinuxNotificationLocal(title: string, message: string): Promise<void> {
  try {
    ...
    try {
      ...
      logger.info('Linux notification sent via zenity', { title });
    } catch {
      logger.error(
...
async function playSound(soundFile?: string): Promise<void> {
  ...
  try {
    switch (platform) {
      ...
      default:
        logger.debug('Sound playback not supported on platform', { platform });
    }
  } catch (error) {
...
async function sendPushNotification(
  ...
  switch (platform) {
    ...
    default:
      logger.debug('Notifications not supported on platform', { platform });
  }
}
