...
/**
 * Ratchet state lives in its own table now, but the reads that feed derived
 * state and the snapshot stream consumed it as flat `ratchet*` fields on the
 * workspace. `withRatchet` keeps that shape, so the split stops at this
 * accessor instead of rippling out through derived state, the snapshot wire and
 * the client.
 */
type WithRatchet<T> = Omit<T, 'ratchet'> & WorkspaceRatchetFields;

function withRatchet<T extends { ratchet?: WorkspaceRatchetRow | null }>(row: T): WithRatchet<T> {
  const { ratchet, ...rest } = row;
  return { ...rest, ...flattenWorkspaceRatchet(ratchet) };
}

/** A bare workspace row with its ratchet fields flattened on. */
export type WorkspaceWithRatchet = WithRatchet<
  Prisma.WorkspaceGetPayload<{ include: { ratchet: true } }>
>;

type WorkspaceWithAgentSessions = WithRatchet<
...
type WorkspaceWithAgentSessionsAndProject = WithRatchet<
  Prisma.WorkspaceGetPayload<{
    include: { agentSessions: true; terminalSessions: true; project: true; ratchet: true };
  }>
>;
...
class WorkspaceAccessor {
  async create(data: CreateWorkspaceInput): Promise<WorkspaceWithRatchet> {
    const created = await prisma.workspace.create({
      data: {
        ...
        // Every workspace gets its ratchet row here, so no later code path can
        // create a workspace that the ratchet's row-guarded writes would skip.
  ...
  async findById(id: string): Promise<WorkspaceWithSessions | null> {
    ...
    return row ? withRatchet(row) : null;
  }
  ...
  private async applyPrAggregateUpdateWithDispatchReset(
    ...
    return await prisma.$transaction(async (transaction) => {
      ...
      const applied = await transaction.workspace.updateMany({
  ...
  async findChildrenWithStatus(parentId: string): Promise<
    WithRatchet<
      Prisma.WorkspaceGetPayload<{
        include: { agentSessions: true; project: true; ratchet: true };
      }>
    >[]
  > {
    const rows = await prisma.workspace.findMany({
