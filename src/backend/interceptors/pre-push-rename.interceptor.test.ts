...
describe('prePushRenameInterceptor', () => {
  ...
  it('renames once when two pushes race through the same interceptor', async () => {
    const interceptor = createPrePushRenameInterceptor();
    const raceContext = { ...context, workspaceId: 'ws-race' };

    // Hold the first DB read open so both calls are in flight simultaneously.
    // A claim taken after this await would let both callers rename.
    ...
    const second = interceptor.onToolStart!(
      createEvent({ toolUseId: 'race-b', input: { command: 'git push' } }),
      raceContext
    );
    ...
    expect(mockSetBranchNameAndClearAutoGenerated).toHaveBeenCalledTimes(1);
  });
