...
const mockSetWorkspaceName = vi.hoisted(() => vi.fn());
...
vi.mock('@/backend/domains/workspace', () => ({
  workspaceDataService: {
    ...
    setWorkspaceName: (...args: unknown[]) => mockSetWorkspaceName(...args),
  },
}));
...
describe('branchRenameInterceptor', () => {
  ...
  it('updates workspace branch name after git branch -m', async () => {
    ...
    mockFindById.mockResolvedValue({ id: 'w1', isAutoGeneratedName: false });
    ...
    expect(mockFindById).toHaveBeenCalledWith('w1');
    expect(mockSetWorkspaceName).not.toHaveBeenCalled();
  });

  it('updates workspace name when it was auto-generated', async () => {
    mockExtractInputValue.mockReturnValue('git branch -m user/fix-login-bug');
    mockGitCommand.mockResolvedValue({ code: 0, stdout: 'user/fix-login-bug\n', stderr: '' });
    ...
    expect(mockGitCommand).toHaveBeenCalledWith(['rev-parse', '--abbrev-ref', 'HEAD'], '/tmp/w1');
    expect(mockSetBranchName).toHaveBeenCalledWith('w1', 'user/fix-login-bug');
    expect(mockFindById).toHaveBeenCalledWith('w1');
    expect(mockSetWorkspaceName).toHaveBeenCalledWith('w1', 'Fix Login Bug');
  });
