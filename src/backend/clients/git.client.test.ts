...
const mockGitStateInvalidateAll = vi.hoisted(() => vi.fn());
...
vi.mock('@/backend/services/workspace-git-state.service', () => ({
  workspaceGitStateService: {
    ...
    invalidateAll: mockGitStateInvalidateAll,
  },
}));
...
describe('GitClient', () => {
  ...
  describe('git command wrappers', () => {
    ...
    it('invalidates after successful push-with-upstream and fetch operations', async () => {
      mockGitCommand.mockResolvedValue(commandSuccess());

      await client.pushBranchWithUpstream('/tmp/ws');
      await client.fetch('/tmp/ws');

      expect(mockGitStateInvalidate).toHaveBeenCalledTimes(1);
      expect(mockGitStateInvalidate).toHaveBeenCalledWith('/tmp/ws');
      expect(mockGitStateInvalidateAll).toHaveBeenCalledTimes(1);
    });
