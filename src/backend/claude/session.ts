    ...
    try {
      ...
      for (const line of lines) {
        try {
          ...
          logger.debug('Skipping malformed JSONL line', { sessionId });
        }
      }
    } catch (error) {
      // Return empty array if file doesn't exist or can't be read
      if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
        logger.warn('Error reading session history', { sessionId, error });
      }
    }
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          logger.debug('Could not stat session file', { filePath });
        }
      }
    } catch (error) {
