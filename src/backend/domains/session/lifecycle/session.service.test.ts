...
describe('SessionService', () => {
  ...
  it('applies Codex ratchet non-interactive startup execution mode using YOLO', async () => {
    ...
    const acpHandle = unsafeCoerce<AcpProcessHandle>({
      getPid: vi.fn().mockReturnValue(458),
      provider: 'CODEX',
    ...
    await sessionService.startSession('session-codex-ratchet', {
      initialPrompt: '',
      startupModePreset: 'non_interactive',
    });

    expect(acpRuntimeManager.setSessionMode).toHaveBeenCalledWith('session-codex-ratchet', 'code');
  ...
  it('continues startup when persisting startup mode preset config snapshot fails', async () => {
    const session = unsafeCoerce<
      NonNullable<Awaited<ReturnType<typeof sessionRepository.getSessionById>>>
    >({
      id: 'session-1',
    ...
    const acpHandle = unsafeCoerce<AcpProcessHandle>({
      getPid: vi.fn().mockReturnValue(459),
      provider: 'CLAUDE',
      providerSessionId: 'provider-session-1',
      isPromptInFlight: false,
      configOptions: [modeConfig],
    ...
    const persistSpy = vi
      .spyOn(
        sessionService as unknown as {
          persistAcpConfigSnapshot: (sessionId: string, payload: unknown) => Promise<void>;
        },
    ...
    expect(acpHandle.configOptions[0]?.currentValue).toBe('bypassPermissions');
    expect(acpRuntimeManager.sendPrompt).not.toHaveBeenCalled();
  });
