...
import { chatMessageHandlerService } from '@/backend/services/chat-message-handlers.service';
...
class WorktreeLifecycleService {
  ...
  private async getCachedGitHubUsername(): Promise<string | null> {
    if (this.cachedGitHubUsername === undefined) {
      this.cachedGitHubUsername = await githubCLIService.getAuthenticatedUsername();
    }
    return this.cachedGitHubUsername ?? null;
  }
  ...
  async initializeWorkspaceWorktree(
    ...
    try {
      ...
      const useExistingBranch =
        ...
        (await this.getInitMode(workspaceId, project.worktreeBasePath)) ??
        false;
      ...
      const worktreeInfo = useExistingBranch
        ...
        : await (async () => {
            const gitHubUsername = await this.getCachedGitHubUsername();
      ...
      if (worktreeCreated) {
        await this.clearInitMode(workspaceId, project?.worktreeBasePath);
      }
    }
  }
}
