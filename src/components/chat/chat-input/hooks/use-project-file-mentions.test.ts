...
import { useProjectFileMentions } from './use-project-file-mentions';
...
type FileMentionsResult = ReturnType<typeof useProjectFileMentions>;
...
function Harness({ inputRef, onChange, onResult }: HarnessProps) {
  const result = useProjectFileMentions({
    projectId: 'project-1',
    inputRef,
    onChange,
  });
...
describe('useProjectFileMentions selection', () => {
