...
Purpose: This plan delivers the user-visible features: permission requests with 4 distinct option buttons (allow once, allow always, deny once, deny always), a structured task list for ACP plan updates, and clickable file location links on tool progress events. After this plan, all Phase 20 success criteria are met.

Output: Updated permission prompt with ACP options, new ACP plan view component, clickable tool locations, updated reducer and actions.
</objective>
...
**2. Update tool progress reducer to store acpLocations (`reducer/slices/tooling.ts`):**

Update the `SDK_TOOL_PROGRESS` case to read `acpLocations` and `acpKind` from the action payload:
```typescript
...
Also update the `SDK_TOOL_PROGRESS` action type in `types.ts` to include optional ACP fields:
```typescript
...
Update the `handleToolProgressMessage` in `reducer/index.ts` to pass through `acpLocations` and `acpKind`:
```typescript
...
**4. Create `AcpPlanView` component in `src/components/chat/acp-plan-view.tsx`:**
...
In the section where AgentLiveDock renders tool progress items, check for `acpLocations` on each `ToolProgressInfo`. When present and non-empty, render file paths as clickable links below the tool name:
...
If AgentLiveDock or its parent already has a file-open mechanism (e.g., an `onOpenFile` callback), use that instead of `window.dispatchEvent`. Check the existing code for any file-opening patterns and use the same approach for consistency.

c) Find where AgentLiveDock is used (search for `<AgentLiveDock` in the codebase) and pass `acpPlan={state.acpPlan}` from the chat state. This is likely in `chat-panel.tsx` or a similar parent component that has access to the reducer state. Ensure the `acpPlan` prop is passed from `ChatState` through to the component.
  </action>
  <verify>
Run `pnpm typecheck` -- no type errors.
Run `pnpm check:fix` -- lint clean.
Verify that AcpPlanView is imported and rendered in agent-live-dock.tsx (grep for "AcpPlanView").
Verify that the parent of AgentLiveDock passes `acpPlan` prop (grep for "acpPlan" in the parent component).
Verify that tool progress rendering includes acpLocations clickable links (grep for "acpLocations" in agent-live-dock.tsx).
  </verify>
  <done>
ACP plan updates render as a structured collapsible task list with status icons (pending/in_progress/completed) and priority badges (high/medium/low) in the agent live dock. Tool progress events with acpLocations render file paths as clickable links. Plan state resets on session switch. AcpPlanView is wired into AgentLiveDock via acpPlan prop passed from ChatState.
  </done>
</task>
...
4. Slash commands forwarding (via available_commands_update to slash_commands delta)
5. Thinking content forwarding (via agent_thought_chunk to thinking content_block)
6. Full tool call lifecycle (pending/in_progress/completed/failed with kind and locations)
...
5. **Tool calls**: Verify tool calls appear in the chat UI with status lifecycle indicators. Check that file locations (if present) appear as clickable links below the tool name.
6. **Thinking**: If the agent uses extended thinking, verify thoughts appear in a collapsible section in the live dock
7. **Plans**: If the agent creates a plan, verify it renders as a structured task list with checkmarks and status indicators in the agent live dock
...
9. **Legacy sessions**: Start a regular Claude session (not ACP) and verify the existing binary Allow/Deny permission UI still works correctly
  ...
  <done>All Phase 20 features verified working end-to-end: ACP permission options, plan view, clickable file locations, thinking, tool lifecycle, slash commands, and legacy session compatibility.</done>
</task>
...
<success_criteria>
- Phase 20 Success Criteria 1: Tool calls from agent appear with status lifecycle, title, kind, content, and clickable file locations -- verified visually
