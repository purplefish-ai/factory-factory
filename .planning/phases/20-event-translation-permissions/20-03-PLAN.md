...
must_haves:
  truths:
    ...
    - "Slash commands from available_commands_update appear in the command palette with name, description, and argument hint"
    - "Tool progress events with acpLocations render file paths as clickable links that open files"
  artifacts:
    ...
    - path: "src/components/chat/reducer/slices/tooling.ts"
      provides: "Updated tool progress reducer storing acpLocations and acpKind"
  ...
  key_links:
    ...
    - from: "src/components/chat/acp-plan-view.tsx"
      ...
      pattern: "acpPlan"
    - from: "src/components/chat/agent-live-dock.tsx"
      to: "src/components/chat/acp-plan-view.tsx"
      via: "renders AcpPlanView when acpPlan prop is non-null"
      pattern: "AcpPlanView"
---

<objective>
Build the frontend components for ACP multi-option permissions, structured plan view, and clickable file locations, completing the end-to-end UI for Phase 20.

Purpose: This plan delivers the user-visible features: permission requests with
4 distinct option buttons (allow once, allow always, deny once, deny always), a
structured task list for ACP plan updates, and clickable file location links on
tool progress events. After this plan, all Phase 20 success criteria are met.

Output: Updated permission prompt with ACP options, new ACP plan view component,
clickable tool locations, updated reducer and actions. </objective>
...
This is backward-compatible -- existing callers (PlanApprovalPrompt,
PermissionPromptExpanded) pass only 2 args.

**2. Add AcpPermissionPrompt component in `permission-prompt.tsx`:**

Create a new component `AcpPermissionPrompt` that renders when
`permission.acpOptions` is present. Place it in the same file, above the main
`PermissionPrompt` export.
...
Find the `handleApprovePermission` callback (or similarly named). It currently
builds:
...
**5. Update `reducer/types.ts` to pass optionId through PERMISSION_RESPONSE if
needed:**

The `PERMISSION_RESPONSE` action payload currently only has
`{ allow: boolean }`. This is fine as-is since `optionId` is only needed for the
WebSocket send (handled in use-chat-actions), not for reducer state. No change
needed to the action type.

**6. Verify the `reduceRequestSlice` in `reducer/slices/requests.ts`:**

The existing `WS_PERMISSION_REQUEST` case sets
`pendingRequest: { type: 'permission', request: action.payload }`. Since
`PermissionRequest` (from `@/lib/chat-protocol`) now includes `acpOptions` (from
the shared type changes in Plan 02), the `acpOptions` will automatically flow
through to state. No reducer changes needed -- the type system carries it.

Verify that the `handlePermissionRequestMessage` in `reducer/index.ts` passes
through `acpOptions`. Currently it extracts fields manually:
...
</action> <verify> Run `pnpm typecheck` -- no type errors. Run `pnpm check:fix`
-- lint clean. Visually: The binary Allow/Deny UI still works for Claude/Codex
sessions (no acpOptions). The ACP multi-option UI renders when acpOptions is
present. </verify> <done> ACP permission requests render 4 option buttons (allow
once, allow always, deny once, deny always). Clicking an option sends the
optionId back via WebSocket. Legacy binary Allow/Deny UI unchanged. </done>
</task>

<task type="auto">
  <name>Task 2: ACP plan view, clickable tool locations, and plan state management</name>
  <files>
    src/components/chat/acp-plan-view.tsx
    src/components/chat/reducer/types.ts
    src/components/chat/reducer/slices/tooling.ts
  ...
  <action>
**1. Add ACP plan state and tool location types to ChatState in `reducer/types.ts`:**

Add new types and a new field to `ChatState`:
...
Update the `ToolProgressInfo` interface to carry ACP location data:
...
**2. Update tool progress reducer to store acpLocations
(`reducer/slices/tooling.ts`):**

Update the `SDK_TOOL_PROGRESS` case to read `acpLocations` and `acpKind` from
the action payload:
...
Also update the `SDK_TOOL_PROGRESS` action type in `types.ts` to include
optional ACP fields:

```typescript
| { type: 'SDK_TOOL_PROGRESS'; payload: {
    toolUseId: string;
    toolName: string;
    elapsedSeconds: number;
    acpLocations?: AcpToolLocation[];
...
Update the `handleToolProgressMessage` in `reducer/index.ts` to pass through
`acpLocations` and `acpKind`:

```typescript
function handleToolProgressMessage(data: WebSocketMessage): ChatAction | null {
  // ... existing code to extract toolUseId, toolName, elapsedSeconds ...
  return {
    ...
    payload: {
      ...
      toolName,
      elapsedSeconds,
      // Pass through ACP-specific fields from the tool_progress WebSocket message
...
**3. Handle ACP plan updates in `reducer/index.ts`:**

The ACP plan comes as a `task_notification` with a JSON message containing
`{ type: 'acp_plan', entries: [...] }`. Update `handleTaskNotificationMessage`
to detect and dispatch ACP plan updates:
...
**4. Create `AcpPlanView` component in
`src/components/chat/acp-plan-view.tsx`:**

A collapsible task list component that renders ACP plan entries with status
indicators. Simple, inline in the chat area (similar to how thinking is shown in
AgentLiveDock).
...
**5. Wire AcpPlanView and clickable tool locations into AgentLiveDock:**

In `src/components/chat/agent-live-dock.tsx`:

a) Add `acpPlan` prop and render `AcpPlanView`:
...
Import `AcpPlanView` and `AcpPlanState`:
...
b) Update tool progress rendering to show clickable file locations:

In the section where AgentLiveDock renders tool progress items, check for
`acpLocations` on each `ToolProgressInfo`. When present and non-empty, render
file paths as clickable links below the tool name:
  ...
  <div className="flex flex-wrap gap-1 mt-0.5">
    {progress.acpLocations.map((loc, i) => (
      <button
        ...
        onClick={() => {
          // Send open-file request via WebSocket or use existing file-open mechanism
          // The path is relative to workspace; use the existing openFile action if available,
          // or dispatch a custom event that the parent handles
          window.dispatchEvent(new CustomEvent('acp-open-file', {
...
If AgentLiveDock or its parent already has a file-open mechanism (e.g., an
`onOpenFile` callback), use that instead of `window.dispatchEvent`. Check the
existing code for any file-opening patterns and use the same approach for
consistency.

c) Find where AgentLiveDock is used (search for `<AgentLiveDock` in the
codebase) and pass `acpPlan={state.acpPlan}` from the chat state. This is likely
in `chat-panel.tsx` or a similar parent component that has access to the reducer
state. Ensure the `acpPlan` prop is passed from `ChatState` through to the
component. </action> <verify> Run `pnpm typecheck` -- no type errors. Run
`pnpm check:fix` -- lint clean. Verify that AcpPlanView is imported and rendered
in agent-live-dock.tsx (grep for "AcpPlanView"). Verify that the parent of
AgentLiveDock passes `acpPlan` prop (grep for "acpPlan" in the parent
component). Verify that tool progress rendering includes acpLocations clickable
links (grep for "acpLocations" in agent-live-dock.tsx). </verify> <done> ACP
plan updates render as a structured collapsible task list with status icons
(pending/in_progress/completed) and priority badges (high/medium/low) in the
agent live dock. Tool progress events with acpLocations render file paths as
clickable links. Plan state resets on session switch. AcpPlanView is wired into
AgentLiveDock via acpPlan prop passed from ChatState. </done> </task>
...
1. ACP multi-option permission UI (4 buttons: allow once, allow always, deny
   once, deny always)
2. ACP plan view component (structured task list with status/priority
   indicators)
3. Clickable file locations on tool calls (acpLocations rendered as links)
4. Slash commands forwarding (via available_commands_update to slash_commands
   delta)
5. Thinking content forwarding (via agent_thought_chunk to thinking
   content_block)
6. Full tool call lifecycle (pending/in_progress/completed/failed with kind and
   locations)

Verification steps:

1. Start the dev server: `pnpm dev`
2. Create a test ACP session with `useAcp: true` and `permissionMode: 'plan'`
   (not bypassPermissions)
3. Send a prompt that triggers tool usage (e.g., "Read the README.md file")
4. **Permission UI**: Verify that when the agent requests permission, you see 4
   distinct option buttons (not binary Allow/Deny). Click one -- the agent
   should proceed.
5. **Tool calls**: Verify tool calls appear in the chat UI with status lifecycle
   indicators. Check that file locations (if present) appear as clickable links
   below the tool name.
6. **Thinking**: If the agent uses extended thinking, verify thoughts appear in
   a collapsible section in the live dock
7. **Plans**: If the agent creates a plan, verify it renders as a structured
   task list with checkmarks and status indicators in the agent live dock
8. **Slash commands**: Type `/` in the chat input and verify commands appear in
   the palette
9. **Legacy sessions**: Start a regular Claude session (not ACP) and verify the
existing binary Allow/Deny permission UI still works correctly
  </action>

<verify>Type "approved" or describe issues</verify> <done>All Phase 20 features
verified working end-to-end: ACP permission options, plan view, clickable file
locations, thinking, tool lifecycle, slash commands, and legacy session
...
8. Tool progress with acpLocations renders clickable file links
</verification>

<success_criteria>

- Phase 20 Success Criteria 1: Tool calls from agent appear with status
  lifecycle, title, kind, content, and clickable file locations -- verified
  visually
- Phase 20 Success Criteria 2: Agent thinking renders in collapsible sections --
  verified visually
- Phase 20 Success Criteria 3: Plan updates render as structured task view with
  per-task status -- verified visually
- Phase 20 Success Criteria 4: Permission requests present 4 option buttons,
  selected option flows back as optionId -- verified visually and functionally
- Phase 20 Success Criteria 5: Slash commands appear in command palette --
  verified visually
