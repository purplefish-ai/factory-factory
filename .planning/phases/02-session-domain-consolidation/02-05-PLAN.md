...
Purpose: Chat services manage WebSocket connections for session interaction,
forward Claude CLI events to clients, and dispatch inbound messages. They are
tightly coupled to the session lifecycle and belong in the session domain. This
is the final batch of file moves.

Output: `src/backend/domains/session/chat/` with all chat service files and the
handlers subdirectory. DOM-04 fix for chatWsMsgCounter. Re-export shims at old
paths. </objective>
...
<success_criteria> All chat services and handlers migrated to session domain.
DOM-04 chatWsMsgCounter fixed. Old paths have re-export shims. Full test suite
