...
Purpose: These three services form the middle dependency layer -- they depend on
resource accessors and Prisma types but NOT on other workspace services. Moving
them establishes the workspace domain's `lifecycle/` subdirectory. None require
DOM-04 refactoring (activity already uses instance-based state, state-machine
and data have no module-level state).

Output: `src/backend/domains/workspace/lifecycle/` with three source files and
two test files (data.service has no tests -- it's a 33-line thin wrapper).
...
<success_criteria> Three lifecycle services (state-machine, data, activity) and
their tests are established at src/backend/domains/workspace/lifecycle/. All
existing code compiles via re-export shims. pnpm typecheck passes.
</success_criteria>
