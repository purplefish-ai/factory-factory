...
**Phase Goal:** Consolidate terminal management into
...
- Empty implementations (return null/{}[]): Only legitimate guard clauses
  (getTerminal returns null when workspace not found, getTerminalsForWorkspace
  returns [] when no terminals exist — both are correct behavior)
...
The terminal domain is a pure backend service with no UI components. All
functionality can be verified through:
...
Phase 6 Plan 1 successfully achieved its goal: **Terminal domain consolidation
is complete.**
...
- `5d61d79` — feat(06-01): move terminal.service.ts to domain, create shim and
  barrel (567 insertions, 544 deletions)
- `c3f571d` — test(06-01): add terminal domain unit tests and barrel smoke test
  (379 insertions)

Both commits exist in git history and match the deliverables documented in
SUMMARY.md.
