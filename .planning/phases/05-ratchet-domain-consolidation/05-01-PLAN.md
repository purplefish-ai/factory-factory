...
Purpose: These three services are the leaf/internal services of the ratchet
domain. fixer-session has no intra-ratchet dependencies, ci-fixer depends on
fixer-session, and ci-monitor depends on ci-fixer. Moving them first establishes
the domain files that ratchet.service.ts (Plan 02) will import via relative
paths.

Output: Three source files and two test files in `src/backend/domains/ratchet/`.
Re-export shims at all old paths. </objective>
...
<success_criteria> Three internal ratchet services (fixer-session, ci-fixer,
ci-monitor) and their tests are established at src/backend/domains/ratchet/.
Intra-domain relative imports used for internal dependencies. All existing code
compiles via re-export shims. pnpm typecheck passes. Existing tests pass at new
locations. </success_criteria>
