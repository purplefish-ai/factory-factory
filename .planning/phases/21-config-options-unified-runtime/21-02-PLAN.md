...
Purpose: Render ACP agent-provided config options (model, mode, thought level,
custom) as grouped selectors in the chat input bar, handle config_options_update
messages reactively, and send set_config_option messages when user selects an
option.

Output: AcpConfigSelector component, reducer with acpConfigOptions state,
WebSocket message handling, chat input integration. </objective>
...
- Config option selectors appear in chat input when ACP config options are
  populated
