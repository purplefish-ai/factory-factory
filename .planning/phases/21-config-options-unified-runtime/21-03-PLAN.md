...
Purpose: RUNTIME-08 -- make AcpRuntimeManager the sole runtime for creating new
sessions. Both Claude and Codex sessions route through ACP adapters. Legacy
runtime managers (ClaudeRuntimeManager, CodexAppServerManager) are no longer
used for new session creation.

Output: SessionService creates all sessions via ACP, useAcp flag removed, legacy
manager creation paths removed. </objective>
